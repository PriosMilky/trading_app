    ...
    try:
        ...
        if entry_price > ema20 and ema20 > ema200:
            if rsi < 70:
                ...
                tp = round(entry_price + (0.5 * atr), 2)  # Target TP Scalping Cepat
                
                pips_sl = int(round((entry_price - sl) * 100))
                pips_tp = int(round((tp - entry_price) * 100))
                
                # Perhitungan Cent (Format desimal seperti 1.40 Cent)
                loss_cent = round(pips_sl * lot_size * 0.1, 2)
                profit_cent = round(pips_tp * lot_size * 0.1, 2)
                
                reason = f"Bullish 15M: Harga > EMA 20 & EMA 200. RSI ({rsi}) aman untuk BUY."
            else:
                reason = f"Tren Naik, tapi RSI ({rsi}) sudah Overbought (> 70)."
            ...
            if rsi > 30:
                ...
                reason = f"Bearish 15M: Harga < EMA 20 & EMA 200. RSI ({rsi}) aman untuk SELL."
            else:
                reason = f"Tren Turun, tapi RSI ({rsi}) sudah Oversold (< 30)."
            
        else:
            reason = "Pasar 15M sedang konsolidasi/sideways. Belum ada momentum."
